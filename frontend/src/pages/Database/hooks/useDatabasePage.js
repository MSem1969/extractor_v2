// =============================================================================
// TO_EXTRACTOR v7.0 - USE DATABASE PAGE HOOK
// =============================================================================
...
export function useDatabasePage(currentUser, onOpenOrdine) {
  ...
  const handleBatchArchivia = useCallback(async () => {
    ...
    const conferma = window.confirm(
      ...
      `Lo stato diventera EVASO e non saranno piu modificabili.`
    );
  ...
  const handleBatchValidate = useCallback(async () => {
    ...
    const conferma = prompt(
      `SEI SICURO DI VOLER CONVALIDARE MASSIVAMENTE?\n\n` +
      `Saranno generati i tracciati TOTALI di tutti i ${selected.length} ordini.\n\n` +
      `Se vuoi procedere digita S, altrimenti clicca Annulla:`
    );

    if (conferma?.toUpperCase() !== 'S') return;

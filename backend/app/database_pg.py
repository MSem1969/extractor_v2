...
def get_criterio_by_pattern(pattern_signature: str) -> Optional[Dict[str, Any]]:
    """Ritorna criterio espositore per pattern signature."""
    db = get_db()
    row = db.execute(
...
def get_criteri_stats() -> Dict[str, Any]:
    """Ritorna statistiche criteri (espositore + listino)."""
    db = get_db()
    ...
    stats = {
        'totale_pattern': esp_totale + lst_totale,
        'pattern_ordinari': esp_ordinari + lst_ordinari,
        'pattern_in_apprendimento': esp_in_apprendimento + lst_in_apprendimento,

...
def popola_header_da_anagrafica(id_testata: int, operatore: str = None) -> bool:
    ...
    v7.0: Aggiunto audit trail per tracciare le modifiche.
    ...
    # v7.0: Recupera valori correnti PRIMA dell'aggiornamento per audit
    ordine = db.execute("""
    ...
    # v7.0: Prepara modifiche per audit (solo campi effettivamente modificati)
    modifiche = {}
    valori_precedenti = {
    ...
    db.execute("""
        ...
        SET codice_ministeriale_estratto = COALESCE(NULLIF(%s, ''), codice_ministeriale_estratto),
            ...
            -- v7.0: Aggiorna fonte e timestamp
            fonte_anagrafica = %s,
    ...
    # v7.0: Registra audit per ogni campo modificato
    if modifiche:
        # Aggiungi anche il cambio di fonte_anagrafica
        ...
        try:
            ...
            # Non bloccare se audit fallisce (colonne potrebbero non esistere ancora)
            print(f"⚠️ Audit log fallito: {e}")

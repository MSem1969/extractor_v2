# =============================================================================
# TO_EXTRACTOR v7.0 - LISTINI VENDOR SERVICE
# =============================================================================
...
VENDOR_CSV_MAPPINGS = {
    'CODIFI': {
        ...
        # Prezzi
        'prezzo_pubblico_csv': 'AFPEU1',  # Prezzo pubblico formato XXXXXXYY (2 dec impliciti)
...
def scorporo_iva(prezzo_ivato: float, aliquota_iva: float) -> Optional[float]:
    ...
    Arrotonda per ECCESSO al secondo decimale.
    ...
    Returns:
        Prezzo netto arrotondato per eccesso a 2 decimali
    """
    ...
    # Arrotonda per eccesso a 2 decimali
    return ceil_decimal(prezzo_netto, 2)
    ...
    try:
        ...
        for row_num, row in enumerate(reader, start=2):
            try:
                ...
                # Prezzi da CSV
                prezzo_pubblico_csv = parse_prezzo_intero(row.get(mapping.get('prezzo_pubblico_csv', ''), ''))
...
def arricchisci_riga_con_listino(
    ...
    # v10.3: Escludi righe SC.MERCE, P.O.P., omaggi e espositori dalla verifica listino
    ...
    if tipo_riga in ('SCONTO_MERCE', 'MATERIALE_POP', 'PARENT_ESPOSITORE', 'CHILD_ESPOSITORE'):
        riga['fonte_prezzi'] = 'ESPOSITORE' if 'ESPOSITORE' in tipo_riga else 'OMAGGIO'
        return riga, None

    # Escludi espositori per flag
    if riga.get('is_espositore') or riga.get('is_child'):
        riga['fonte_prezzi'] = 'ESPOSITORE'
        return riga, None
    ...
    listino = get_prezzo_listino(codice_aic, vendor)
    ...
    if not listino:
        ...
        anomalia = {
            ...
            'descrizione': f'Codice AIC {codice_aic} non trovato nel listino {vendor}',
...
# Vendor che richiedono lookup listino per i prezzi
VENDORS_RICHIEDONO_LISTINO = {'CODIFI'}

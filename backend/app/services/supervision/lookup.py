# =============================================================================
# TO_EXTRACTOR v8.0 - SUPERVISION LOOKUP
# =============================================================================
...
def calcola_pattern_signature_lookup(
    ...
    Pattern: VENDOR|CODICE_ANOMALIA|PARTITA_IVA
    ...
    Returns:
        Hash 16 caratteri
    """
    raw = f"{vendor or 'UNKNOWN'}|{codice_anomalia}|{partita_iva or 'NO_PIVA'}"
...
def crea_supervisione_lookup(
    ...
    """
    Crea supervisione per anomalia lookup (LKP-A01, LKP-A02).
    ...
    # Calcola pattern signature per lookup
    pattern_sig = calcola_pattern_signature_lookup(vendor, codice_anomalia, partita_iva)
...
def registra_approvazione_pattern_lookup(pattern_signature: str, operatore: str):
    ...
    # Verifica promozione
    row = db.execute(
        "SELECT count_approvazioni FROM criteri_ordinari_lookup WHERE pattern_signature = %s",
        (pattern_signature,)
    ).fetchone()
    ...
    if row and row[0] >= SOGLIA_PROMOZIONE:
        ...
        log_operation(
            ...
            None,
            f"Pattern lookup {pattern_signature} promosso a ordinario dopo {SOGLIA_PROMOZIONE} approvazioni"
        )

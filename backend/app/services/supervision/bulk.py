# =============================================================================
# TO_EXTRACTOR v8.0 - SUPERVISION BULK OPERATIONS
# =============================================================================
...
def approva_pattern_bulk(
    ...
    3. Incrementa il contatore pattern UNA SOLA VOLTA
       (non N volte per N supervisioni)
    ...
    results['total'] = results['espositore'] + results['listino'] + results['lookup']
    results['orders_affected'] = list(results['orders_affected'])

    # 4. Incrementa pattern UNA SOLA VOLTA
    if results['total'] > 0:
        _incrementa_pattern_una_volta(pattern_signature, operatore)
    ...
    log_operation(
        ...
        f"({results['espositore']} ESP, {results['listino']} LST, {results['lookup']} LKP), "
...
def rifiuta_pattern_bulk(
    ...
    # 4. Reset pattern UNA SOLA VOLTA
    if results['total'] > 0:
...
def _incrementa_pattern_una_volta(pattern_signature: str, operatore: str):
    """
    Incrementa contatore pattern in tutte le tabelle criteri.
    Chiamato una sola volta per bulk approval.
    """

...
from .listini import arricchisci_ordine_con_listino, VENDORS_RICHIEDONO_LISTINO
...
def process_pdf(
    ...
    try:
        ...
        # =====================================================================
        # 7.5 ARRICCHIMENTO LISTINO (v7.0 - Solo per vendor che lo richiedono)
        # =====================================================================
        if vendor in VENDORS_RICHIEDONO_LISTINO:
...
def _insert_order(
    ...
    if lookup_method == 'NESSUNO':
        ...
        anomalia_lkp = {
            ...
            'vendor': vendor_rilevato or 'UNKNOWN',
...
def _insert_detail_row(db, id_testata: int, riga: Dict) -> int:
    ...
    if is_esp and not riga.get('anomalie_espositore'):
        db.execute("""
            ...
            VALUES (%s, %s, 'ESPOSITORE', 'INFO', 'Riga espositore parent elaborata')
        """, (id_testata, id_det))
